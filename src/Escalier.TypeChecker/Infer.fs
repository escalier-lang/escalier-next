...
module rec Infer =
  ...
  let getPatBindingPaths
    (pat: Syntax.Pattern)
    : Map<string, list<string> * bool> =
    let mutable result: Map<string, list<string> * bool> = Map.empty

    let rec walkPattern (pat: Syntax.Pattern) (path: list<string>) =
      match pat.Kind with
      | PatternKind.Ident { Name = name; IsMut = mut } ->
      ...
      | PatternKind.Object object ->
        for elem in object.Elems do
          match elem with
          | Syntax.KeyValuePat { Key = key; Value = value } ->
            walkPattern value (key :: path)
          | Syntax.ShorthandPat { Name = name; IsMut = mut } ->
            result <- Map.add name ((name :: path), mut) result
          | Syntax.RestPat { Target = target; IsMut = mut } ->
  ...
  let checkMutability
    ...
    (init: Expr)
    : Result<list<list<string>>, TypeError> =
    result {
      let mutable invariantPaths = []
